...
from sprocket_mod_manager.infrastructure.log_upload import latest_log_path, upload_latest_log, upload_log_file
...
class LogUploadTests(unittest.TestCase):
    ...
    def test_upload_log_file_accepts_manager_log_path(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "Latest.log"
            path.write_text("manager ready", encoding="utf-8")
            with patch("sprocket_mod_manager.infrastructure.log_upload.urlopen", return_value=_Response()):
                result = upload_log_file(path, "https://logs.example/upload", app_version="1.0.0")
            self.assertEqual(result.bytes_uploaded, len(b"manager ready"))
