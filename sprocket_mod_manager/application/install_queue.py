...
class InstallQueue:
    def __init__(self, runner: QueueRunner, listener: QueueListener | None = None):
        ...
        self._worker.start()
        LOGGER.debug("install queue worker started")
    ...
    def enqueue(
        ...
        LOGGER.info("queued installs count=%d packages=%s", len(added), [entry.package_id for entry in added])
    ...
    def cancel(self, task_id: str) -> bool:
        with self._condition:
            ...
            if entry is None or entry.state != WAITING:
                LOGGER.warning("queue cancellation rejected task_id=%s", task_id)
                return False
    ...
    def close(self, timeout: float | None = 5.0) -> bool:
        LOGGER.debug("closing install queue timeout=%s", timeout)
        with self._condition:
            self._closed = True
            for entry in self._entries:
    ...
    def _work(self) -> None:
        while True:
            with self._condition:
                ...
                snapshot = self._snapshot_locked()
            self._notify(snapshot)
            LOGGER.info("install queue item started task_id=%s package=%s", entry.task_id, entry.package_id)
            ...
            try:
                ...
                LOGGER.exception("install queue item failed task_id=%s package=%s", entry.task_id, entry.package_id)
                with self._condition:
                    entry.state = FAILED
                    entry.message = str(exc)
                    entry.error_code = str(getattr(exc, "code", ""))
                    self._condition.notify_all()
                    snapshot = self._snapshot_locked()
            else:
                with self._condition:
                    ...
                    self._condition.notify_all()
                    snapshot = self._snapshot_locked()
                LOGGER.info("install queue item completed task_id=%s package=%s", entry.task_id, entry.package_id)
            self._notify(snapshot)
    ...
    def _notify(self, snapshot: tuple[InstallQueueEntry, ...]) -> None:
        ...
        try:
            ...
            LOGGER.exception("install queue listener failed")
